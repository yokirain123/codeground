...
import type { Options, FindComponentsResult, MiddlewareRoutingItem, RequestContext, NormalizedRouteManifest, LoadedRenderOpts, RouteHandler, NextEnabledDirectories, BaseRequestHandler } from 'next/dist/server/base-server';
...
import { type IncrementalResponseCacheEntry } from 'next/dist/server/response-cache';
...
import type { PrefetchHints } from 'next/dist/shared/lib/app-router-types';
...
export default class NextNodeServer extends BaseServer<Options, NodeNextRequest, NodeNextResponse> {
    ...
    protected loadInstrumentationModule(): Promise<import("next/dist/server/instrumentation/types").InstrumentationModule | undefined>;
    ...
    protected imageOptimizer(req: NodeNextRequest, res: NodeNextResponse, paramsResult: import('next/dist/server/image-optimizer').ImageParamsResult, previousCacheEntry?: IncrementalResponseCacheEntry | null): Promise<{
        buffer: Buffer;
        contentType: string;
        maxAge: number;
        upstreamEtag: string;
    ...
    revalidate({ urlPath, headers, opts, }: {
        urlPath: string;
        headers: {
