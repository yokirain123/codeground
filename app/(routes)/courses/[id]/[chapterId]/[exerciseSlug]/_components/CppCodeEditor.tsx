...
const MARKER_OWNER = "codequest-cpp-compiler";
const EDITOR_THEME = "codequest-cpp";
...
function parseCompilerDiagnostics(output: string): CompilerDiagnostic[] {
  ...
  const pattern =
    /(?:^|\n)[^\r\n:]*\.(?:cpp|cc|cxx):(\d+):(\d+):\s*(fatal error|error|warning):\s*([^\r\n]+)/gi;

  for (const match of output.matchAll(pattern)) {
    const diagnosticType = match[3].toLowerCase();

    diagnostics.push({
      line: Number(match[1]),
      column: Number(match[2]),
      severity: diagnosticType === "warning" ? "warning" : "error",
      code: diagnosticType === "warning" ? "warning" : "error",
...
export default function CppCodeEditor({
  ...
  const cppEntry = useMemo(() => {
    ...
    return entry ?? ["/main.cpp", ""];
  }, [exercise.starterCode]);

  const [filename, starterCode] = cppEntry;
  const cleanFilename = filename.replace(/^\/+/, "") || "main.cpp";
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/code/run/cpp", {
        method: "POST",
