...
let cachedCSharpLanguageId: number | null = null;
...
async function resolveCSharpLanguageId(baseUrl: string) {
  const configuredId = Number(process.env.JUDGE0_CSHARP_LANGUAGE_ID);

  if (Number.isInteger(configuredId) && configuredId > 0) {
    return configuredId;
  }

  if (cachedCSharpLanguageId) {
    return cachedCSharpLanguageId;
  }
  ...
  const csharpLanguages = languages.filter((language) =>
    /^c#/i.test(language.name),
  );

  const language =
    csharpLanguages.find((item) => /\.net/i.test(item.name)) ??
    csharpLanguages.find((item) => /mono/i.test(item.name)) ??
    csharpLanguages[0];
  ...
  cachedCSharpLanguageId = language.id;
  return language.id;
}
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as RunCSharpBody;
    ...
    const languageId = await resolveCSharpLanguageId(baseUrl);
